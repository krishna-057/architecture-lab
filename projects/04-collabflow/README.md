...
Cookie-backed requests verify `collabflow_session`, require a matching `X-CollabFlow-CSRF` / `collabflow_csrf` double-submit token for mutations, and bind websocket joins to the verified session identity. The session supplies `user_id` and `display_name`; workspace roles still come from the membership table so permission changes apply immediately. Login and invite flows remain deferred.
