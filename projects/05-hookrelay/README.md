...
- `apps/web` Next.js dashboard for creating endpoints, submitting events, searching delivery attempts by server-side filters, replaying attempts, and viewing the delivery contract.
...
| `GET /api/deliveries` | Search recent delivery attempts by status, failure class, endpoint, event id, text query, and limit while returning signature previews, response status, and replay audit fields. |
...
- Saved operator delivery views and long-retention delivery analytics.
