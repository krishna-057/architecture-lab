import { existsSync, readFileSync } from "node:fs";
import { join } from "node:path";

const requiredFiles = [
  "package.json",
  ".env.example",
  "compose.yaml",
  "DELIVERY_CONTRACT.md",
  "db/schema.sql",
  "apps/web/package.json",
  "apps/web/src/app/page.tsx",
  "apps/web/src/app/layout.tsx",
  "services/api/package.json",
  "services/api/Dockerfile",
  "services/api/src/server.js",
  "services/api/src/app.js",
  "services/api/src/config.js",
  "services/api/src/storage.js",
  "services/api/src/observability.js",
  "services/api/src/producer-auth.js",
  "services/api/src/receiver-failure.js",
  "services/api/src/alert-notifier.js",
  "services/api/src/rate-limiter.js",
  "services/api/src/retry-policy.js",
  "services/api/src/delivery-queue.js",
  "services/api/src/delivery-runner.js",
  "services/api/src/worker.js"
];

for (const file of requiredFiles) {
  if (!existsSync(join(process.cwd(), file))) {
    throw new Error(`Missing scaffold file: ${file}`);
  }
}

const rootPackage = JSON.parse(readFileSync("package.json", "utf8"));
if (
  !Array.isArray(rootPackage.workspaces) ||
  !rootPackage.workspaces.includes("apps/*") ||
  !rootPackage.workspaces.includes("services/*")
) {
  throw new Error("Root package.json must declare apps/* and services/* workspaces.");
}

const apiPackage = JSON.parse(readFileSync("services/api/package.json", "utf8"));
if (!apiPackage.dependencies?.fastify) {
  throw new Error("HookRelay API must use Fastify for the first HTTP delivery boundary.");
}

for (const dependency of ["pg", "bullmq", "ioredis"]) {
  if (!apiPackage.dependencies?.[dependency]) {
    throw new Error(`HookRelay API is missing required durable boundary dependency: ${dependency}`);
  }
}

const apiFile = [
  "services/api/src/app.js",
  "services/api/src/config.js",
  "services/api/src/storage.js",
  "services/api/src/observability.js",
  "services/api/src/producer-auth.js",
  "services/api/src/receiver-failure.js",
  "services/api/src/alert-notifier.js",
  "services/api/src/rate-limiter.js",
  "services/api/src/retry-policy.js",
  "services/api/src/delivery-queue.js",
  "services/api/src/delivery-runner.js",
  "services/api/src/worker.js"
].map((file) => readFileSync(file, "utf8")).join("\n");
for (const marker of [
  "/api/endpoints",
  "/api/events",
  "/api/deliveries",
  "/api/deliveries/export",
  "/api/alert-routes",
  "/api/failure-alerts",
  "/api/failure-alerts/:alert_id/acknowledge",
  "/api/failure-alerts/:alert_id/retry-notification",
  "/api/delivery-views",
  "/api/delivery-contract",
  "/api/receiver-verification-example",
  "/api/alert-receiver-verification-example",
  "/api/observability/spans",
  "/api/producer-api-keys",
  "/api/producer-api-keys/:key_id/rotate",
  "/api/producer-api-keys/:key_id/revoke",
  "HookRelay-Signature",
  "Producer API key is required",
  "owner_scoped_api_key",
  "owner_scoped_operator_api_key",
  "producerRoles",
  "replayRoles",
  "required_roles",
  "role must be producer, operator, or admin",
  "owner_id",
  "hashProducerApiKey",
  "rotateProducerApiKey",
  "revokeProducerApiKey",
  "rotated_from_key_id",
  "revoked_at",
  "trace_id",
  "span_id",
  "Endpoint rate limit exceeded",
  "hookrelay.endpoint.rate_limit",
  "rate_limit_per_minute",
  "Retry-After",
  "hookrelay.delivery.process",
  "receiver_failure_classification",
  "delivery_search",
  "delivery_export",
  "deliveryExportMaxRows",
  "deliveryExportColumns",
  "parseDeliveryExportSearch",
  "formatDeliveryExportCsv",
  "listDeliveryExport",
  "receiver_failure_alert_routing",
  "dispatchAlertNotification",
  "dispatchAndRecordAlertNotification",
  "buildAlertNotificationPayload",
  "buildAlertNotificationRequest",
  "buildAlertReceiverVerificationExample",
  "signAlertNotification",
  "HookRelay-Alert-Signature",
  "HookRelay-Alert-Timestamp",
  "alertNotificationSigningSecret",
  "HOOKRELAY_ALERT_NOTIFICATION_SIGNING_SECRET",
  "updateFailureAlertNotification",
  "getFailureAlert",
  "hookrelay.alert.notification",
  "notification_status",
  "notification_response_status",
  "notification_error",
  "notification_attempted_at",
  "notification_attempt_count",
  "notification_next_retry_at",
  "notification_retry_exhausted",
  "alertNotificationRetryDelaysSeconds",
  "notification_retry_endpoint",
  "notification_retry_rule",
  "webhook alert target must be an absolute URL",
  "failureAlertRoles",
  "parseFailureAlertRoute",
  "failureAlertSuppressionMaxSeconds",
  "suppression_window_seconds",
  "suppression_window_max_seconds",
  "suppression_rule",
  "last_alert_at",
  "suppressed_until",
  "alertRouteIsSuppressed",
  "listAlertRoutes",
  "createAlertRoute",
  "deleteAlertRoute",
  "listFailureAlerts",
  "acknowledgeFailureAlert",
  "parseFailureAlertAcknowledgement",
  "acknowledgement_endpoint",
  "acknowledgement_rule",
  "acknowledged_at",
  "acknowledged_by",
  "acknowledgement_note",
  "already_acknowledged",
  "routeFailureAlert",
  "delivery_saved_views",
  "parseDeliveryViewFilters",
  "listDeliveryViews",
  "createDeliveryView",
  "deleteDeliveryView",
  "parseDeliverySearch",
  "parseDeliveryCursor",
  "next_cursor",
  "created_at_desc_delivery_id_desc",
  "classifyReceiverFailure",
  "failure_class",
  "receiver_http_5xx",
  "receiver_timeout",
  "buildReceiverVerificationExample",
  "idempotency_key",
  "replay_authorization",
  "Replay reason",
  "replay",
  "jitter_ratio",
  "calculateRetrySchedule",
  "retry_policy",
  "new Pool",
  "new Queue",
  "new Worker",
  "dead_letter"
]) {
  if (!apiFile.includes(marker)) {
    throw new Error(`API is missing required marker: ${marker}`);
  }
}

const webFile = readFileSync("apps/web/src/app/page.tsx", "utf8");
for (const marker of [
  "/api/endpoints",
  "/api/events",
  "/api/deliveries",
  "/api/delivery-contract",
  "/api/receiver-verification-example",
  "/api/alert-receiver-verification-example",
  "/api/observability/spans",
  "/api/producer-api-keys",
  "Producer Auth",
  "Rotate",
  "Revoke",
  "operator",
  "admin",
  "owner_id",
  "Rate Limit",
  "rate_limit_per_minute",
  "Idempotency Key",
  "Observability",
  "Receiver Verification",
  "Alert Receiver Verification",
  "alertVerificationExample",
  "Replay Auth",
  "Retry Jitter",
  "Failure Class",
  "failureClassFilter",
  "deliveryStatusFilter",
  "Server-side delivery search",
  "Saved Views",
  "Save View",
  "Delete View",
  "Export CSV",
  "Delivery Export",
  "downloadDeliveryExport",
  "Alert Routing",
  "Add Route",
  "Suppress Seconds",
  "alertSuppressionWindowSeconds",
  "suppression_window_seconds",
  "suppressed_until",
  "Acknowledge",
  "notification_status",
  "notification_error",
  "notification_attempt_count",
  "notification_next_retry_at",
  "Retry Notification",
  "retryFailureAlertNotification",
  "Acknowledgement Note",
  "acknowledgementNote",
  "acknowledgeFailureAlert",
  "acknowledged_at",
  "FailureAlertRoute",
  "failureAlerts",
  "DeliverySavedView",
  "savedDeliveryViews",
  "Search Deliveries",
  "Load More",
  "DeliverySearchResponse",
  "No delivery attempts match the delivery search.",
  "Signature Preview",
  "Replay"
]) {
  if (!webFile.includes(marker)) {
    throw new Error(`Web app is missing required marker: ${marker}`);
  }
}

const contract = readFileSync("DELIVERY_CONTRACT.md", "utf8");
for (const marker of [
  "HMAC",
  "Retry Policy",
  "Jitter",
  "Replay Rule",
  "Replay Authorization",
  "Failure Classification",
  "Receiver Verification",
  "Observability",
  "Rate Limit",
  "Producer API Key",
  "rotated",
  "revoked",
  "operator",
  "admin",
  "required_roles",
  "failure_class",
  "server-side `failure_class` filter",
  "server-side delivery search",
  "Delivery Saved Views",
  "Delivery Export",
  "Alert Routing",
  "Alert Acknowledgement",
  "Alert Suppression Windows",
  "External Notification Delivery",
  "receiver_failure_alert_routing",
  "/api/alert-routes",
  "/api/failure-alerts",
  "/api/failure-alerts/:alert_id/acknowledge",
  "/api/failure-alerts/:alert_id/retry-notification",
  "acknowledgement_endpoint",
  "notification_retry_endpoint",
  "notification_retry_delays_seconds",
  "notification_signing",
  "verification_example_endpoint",
  "HookRelay-Alert-Signature",
  "HookRelay-Alert-Timestamp",
  "HOOKRELAY_ALERT_NOTIFICATION_SIGNING_SECRET",
  "notification_attempt_count",
  "notification_next_retry_at",
  "notification_retry_rule",
  "suppression_window_seconds",
  "suppression_window_max_seconds",
  "suppression_rule",
  "notification_status",
  "Notification delivery",
  "webhook",
  "acknowledged_at",
  "local_alert_record_before_external_integrations",
  "delivery_export",
  "/api/deliveries/export",
  "Exports are bounded snapshots",
  "delivery_saved_views",
  "/api/delivery-views",
  "Saved views store filters only",
  "GET /api/deliveries?status=",
  "next_cursor",
  "created_at_desc_delivery_id_desc",
  "opaque_base64url_json",
  "receiver_http_5xx",
  "owner_id",
  "idempotency_key",
  "HookRelay-Signature"
]) {
  if (!contract.includes(marker)) {
    throw new Error(`Delivery contract is missing required marker: ${marker}`);
  }
}

const schema = readFileSync("db/schema.sql", "utf8");
for (const marker of [
  "webhook_endpoints",
  "webhook_events",
  "delivery_attempts",
  "unique (endpoint_id, idempotency_key)",
  "dead_letter",
  "replay_reason",
  "replay_requested_by",
  "base_delay_seconds",
  "jitter_seconds",
  "scheduled_delay_seconds",
  "failure_class",
  "delivery_observability_spans",
  "delivery_saved_views",
  "receiver_failure_alert_routes",
  "receiver_failure_alerts",
  "suppression_window_seconds",
  "last_alert_at",
  "notification_status",
  "notification_response_status",
  "notification_error",
  "notification_attempted_at",
  "notification_attempt_count",
  "notification_next_retry_at",
  "notification_retry_exhausted",
  "acknowledged_at",
  "acknowledged_by",
  "acknowledgement_note",
  "idx_failure_alert_routes_owner_created",
  "idx_failure_alerts_owner_created",
  "idx_delivery_saved_views_owner_created",
  "producer_api_keys",
  "owner_id",
  "role",
  "rotated_from_key_id",
  "revoked_at",
  "rate_limit_per_minute",
  "idx_delivery_attempts_endpoint_created",
  "idx_delivery_attempts_failure_created",
  "idx_delivery_attempts_endpoint_cursor",
  "idx_delivery_attempts_failure_cursor"
]) {
  if (!schema.includes(marker)) {
    throw new Error(`PostgreSQL schema is missing required marker: ${marker}`);
  }
}

const compose = readFileSync("compose.yaml", "utf8");
for (const marker of ["postgres:16-alpine", "redis:7-alpine", "./.data/postgres", "./.data/redis", "worker:"]) {
  if (!compose.includes(marker)) {
    throw new Error(`Compose stack is missing required marker: ${marker}`);
  }
}

console.log("HookRelay scaffold check passed.");
